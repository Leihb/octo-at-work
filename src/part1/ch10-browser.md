...
这一轮 22 次权限框，两分钟。四分钟没进去的门，录一遍三下点击就过去了。
...
| 活 | 时间 | 权限框 |
| --- | --- | --- |
| 圆通公告页（含一次问我） | 约 1 分 | 23 |
| 创作者中心找"笔记管理"（没进去） | 4 分（含我答题） | 43 |
| 开始录制 | 5 秒 | 1 |
| 停止录制 + 读 5 篇数据 + 读评论分类 | 2 分 4 秒 | 22 |
| 回放 1（第 6 步失败，含诊断） | 约 1 分半 | 5 |
| 回放 2（删掉手滑后） | 64 秒 | 2 |
| 回放 3（改坏选择器，自愈） | 1 分 43 秒 | 1 |
| 回放 4（验证） | 54 秒 | 1 |

回放本身几乎不需要模型，那是浏览器在按步骤点。慢的是它自己摸索页面的那几段，
和读回来大段文字再整理的那一段。四分钟找不到菜单，比读完全部数据用的时间还长。
