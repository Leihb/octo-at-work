...
这一轮它读的是上一章那份 09-02 的转录（原因见下面的翻车点），所以正好能跟上一章的产物直接比。
上一章的精简版第一条结论是这么写的：
...
**它没找到你说的文件，自己换了一个。** 我让它读 `transcript-0908.txt`，那份不在它的工作目录里
（我放到了另一个文件夹）。它没有停下来问，选了目录里唯一一份转录做完了整个活，
然后在最后的说明里交代了一句："你提到的 `transcript-0908.txt` 在文件夹里实际叫 `transcript.txt`
（内容是 09-02 的 920 大促启动会），我按它做了纪要。"

它交代得很清楚，但那份"复盘纪要2.md"里装的是上一场会。**文件名对不上的时候，
它倾向于把活做完，而不是停下来问你。** 你只看文件不看它最后那段话，就会拿着一份错的纪要去开会。
第 2 章"要问我的"那一条要加上一句：**我给的文件名找不到，别猜，问我。**
